...
const videoDecs = document.querySelector(".decs");
const videoFiles = document.querySelector(".files");
...
function loadMusic(indexNumb) {
  ...
  videoDecs.innerHTML = `${allVideos[indexNumb - 1].decs}`;
  videoFiles.innerHTML = `${
    allVideos[indexNumb - 1].files
  }.pdf  <a href="#" onclick="openPDF()">Open file</a>`;
  videoLink = `media/${allVideos[indexNumb - 1].link}.pdf`;
  console.log(videoLink);
}
function openPDF() {
  window.open("/media/016 all-coding-challenges.pdf", "_blank");
}
