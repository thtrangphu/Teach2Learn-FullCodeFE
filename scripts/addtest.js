...
window.onload = () => {
  ...
  addtestForm.addEventListener("submit", (e) => {
    ...
    for (let i = 0; i < 6; i++) {
      data[addtestForm[i].name] = addtestForm[i].value;
    }
    data["no_of_questions"] = no_of_questions;
    data["questions"] = [];
    for (let i = 6; i < addtestForm.length - 1; i += 10) {
      // console.log(addtestForm[i].checked);
      let question = {};
      question["question"] = addtestForm[i].value;
      question["marks"] = addtestForm[i + 1].value;
      let options = [];
      for (let j = i + 2; j < i + 10; j += 2) {
        let opt = {};
        opt["is_correct"] = addtestForm[j].checked ? 1 : 0;
        opt["option"] = addtestForm[j + 1].value;
        options.push(opt);
      }
...
function addQuestion() {
  ...
  document.getElementById("addtestForm").appendChild(node);
  no_of_questions++;
}

function deleteQuestion(e) {
  if (no_of_questions == 1) {
    alert("MINIMUM NUMBER OF QUESTION SHOULD BE THERE TO CREATE A test");
  } else {
